...
def calculate_rsi(series, period=14):
    ...
    rsi = 100 - (100 / (1 + rs))
    return rsi
    #rsi assesses the speed and change of price movements to identify potential overbought or oversold conditions in a stock. It helps traders determine if a stock's price is rising or falling too quickly, potentially signaling a trend reversal. RSI values range from 0 to 100, with values above 70 indicating an overbought condition and values below 30 indicating an oversold condition.
...
def passes_oversold_bounce(data):
    return (data["prev_rsi"] < 30 and  # Yesterday was oversold
            data["rsi"] > data["prev_rsi"] and  # RSI rising today (bounce attempt)
            data["close"] > data["open"])

def passes_short_criteria(data):
    return (
        data["volume"] > 0.9 * data["avg_volume"]          # healthy volume
        and data["close"] <= 1.1 * data["low"]           # close is near low = weak close
        and 35 < data["rsi"] < 50                        # weak, but not oversold (smart)
        and 25 < data["stoch_k"] < 40                      # weak stochastic range
        and data["stoch_k"] < data["stoch_d"]              # downward stoch crossover
        and data["macd_line"] < data["macd_signal"]        # bearish crossover
        and data["macd_hist"] < data["prev_hist"]          # worsening momentum
    )
